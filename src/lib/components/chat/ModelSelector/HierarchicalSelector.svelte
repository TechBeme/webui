<script lang="ts">
	...
	// Tag para modelos sem categoria
	let otherTag = 'other';
	$: otherTag = $i18n.t('Other').toLowerCase();

	// Tag especial para assistentes
	let assistantTag = 'assistant';
	$: assistantTag = $i18n.t('Assistant').toLowerCase();
		...
		items
			...
			.forEach((item) => {
				...
				if (modelTags.length > 0) {
					...
					tagMap.get(otherTag)!.models.push(item);
				}
			});

		tagGroups = Array.from(tagMap.values()).sort((a, b) => {
			// "outros" sempre por último
			if (a.tag === otherTag) return 1;
			if (b.tag === otherTag) return -1;

			// "assistant" sempre penúltimo (antes de "outros")
			if (a.tag === assistantTag) return 1;
			if (b.tag === assistantTag) return -1;

			// Demais tags em ordem alfabética
						...
						{#each filteredTagGroups as group}
							...
							>
								{#if group.icon}
									...
									/>
								{:else if group.tag === assistantTag}
									<svg

<script lang="ts">
	...
	const SPECIAL_TAG_RECOMMENDED = '__recommended__';
	...
	$: {
				...
				if (modelTags.length > 0) {
					modelTags.forEach((tag: any) => {
						let tagName = tag.name;
							...
							// Detectar tags especiais e converter para valores internos
							if (tagName.toLowerCase() === 'recommended') {
								tagName = SPECIAL_TAG_RECOMMENDED;
							} else if (tagName.toLowerCase() === 'assistant') {
		...
		tagGroups = Array.from(tagMap.values()).sort((a, b) => {
			// "recomendado" sempre primeiro
			if (a.tag === SPECIAL_TAG_RECOMMENDED) return -1;
			if (b.tag === SPECIAL_TAG_RECOMMENDED) return 1;
	...
	const getDisplayTag = (tag: string): string => {
		...
		if (tag === SPECIAL_TAG_RECOMMENDED) {
			// Tradução inline para não depender do translation.json (sobrescrito pelo CI)
			return ($i18n.language ?? '').startsWith('pt') ? 'Recomendado' : 'Recommended';
		}
	...
	// Hover com delay para evitar troca acidental ao passar o mouse
	let hoverTimer: ReturnType<typeof setTimeout> | null = null;

	const handleCategoryHover = (tag: string) => {
		if ($mobile) return;
		if (hoverTimer) clearTimeout(hoverTimer);
		hoverTimer = setTimeout(() => selectTag(tag), 200);
	};
...
<DropdownMenu.Root
	...
	onOpenChange={async () => {
		searchValue = '';
		selectedTag = tagGroups[0]?.tag ?? null;
			...
			<div class="flex max-h-[calc(100vh-200px)]">
				...
				<div class="w-48 border-r border-gray-200 dark:border-gray-700 overflow-y-auto">
						{#each filteredTagGroups as group}
							...
							on:mouseenter={() => handleCategoryHover(group.tag)}
							on:mouseleave={cancelHover}
							on:click={() => selectTag(group.tag)}
						>
							{#if group.icon && group.tag !== SPECIAL_TAG_RECOMMENDED}
								<img
								...
								/>
							{:else if group.tag === SPECIAL_TAG_RECOMMENDED}
								<svg
